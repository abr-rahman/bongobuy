...
class OrderController extends Controller
{
    ...
    public function approved($id)
    {
        ...
        foreach ($orderItems as $orderList) {
            $inventoryQuery = Inventory::where('product_id', $orderList->product_id);
            if ($orderList->color_id) {
                $inventoryQuery->where('color_id', $orderList->color_id);
            }
            ...
            if($inventory->quantity > 1) {
                ...
                if ($orderList->color_id == null && $orderList->size_id == null) {
                    Inventory::where(['product_id' => $orderList->product_id])->decrement('quantity', $orderList->quantity);
                }
            } else {
    ...
    public function acceptReturn($id)
    {
        ...
        $orderLists = OrderList::select('id', 'product_id', 'size_id', 'color_id', 'quantity', 'order_id', 'product_price')
                    ->where('order_id', $item->id)
                    ->get();
