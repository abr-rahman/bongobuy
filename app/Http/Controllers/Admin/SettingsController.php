...
class SettingsController extends Controller
{
    ...
    public function update(SettingsUpdateRequest $request, $id)
    {
        ...
        if ($request->hasFile('icon')) {
            $oldFile = \public_path('uploads/logo/' . $settings->icon);
            if (isset($settings->icon)) {
                if (file_exists($oldFile)) {
                    if($settings->icon != 'default-logo.png') {
                        unlink($oldFile);
                    }
                }
            }
            $icon = $this->uploader->upload($request->file('icon'), 'uploads/logo/');
            $settings['icon'] = $icon;
            $settings->icon = $icon;
            $settings->save();
        }
        ...
        $settings->youtube_link_two = $request->youtube_link_two;
