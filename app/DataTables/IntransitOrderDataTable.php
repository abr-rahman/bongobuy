...
class IntransitOrderDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                $html .= '<a class="dropdown-item" id="check_status" href="' . route('order.add_return', $row->id) . '">Return</a>';
                $html .= '</div>';
                $html .= '</div>';
    ...
    {
        return $model->newQuery()->where('status', StatusEnum::OnWay->value)->orderBy('created_at', 'desc');
    }
