    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('status', function ($row) {
                ...
                if ($row->status == StatusEnum::Outlet->value) {
                    $html = '<div class="col-sm-5"><a class="btn bg-info badge" >Outlet</a> </div>';
                    return $html;
                }
                if ($row->status == StatusEnum::Bannar->value) {
                    $html = '<div class="col-sm-5"><a class="btn bg-info badge" >Bannar</a> </div>';
                    return $html;
                }
            })
            ->addColumn('action', function ($row) {
                ...
                if ($row->status == StatusEnum::Active->value) {
                    $html .= '<a class="dropdown-item" id="check_status" href="' . route('sliders.add_outlet', $row->id) . '">Add Outlets</a>';
                    $html .= '<a class="dropdown-item" id="check_status" href="' . route('sliders.add_bannar', $row->id) . '">Add Bannar</a>';
                }
                if ($row->status == StatusEnum::Outlet->value) {
                    $html .= '<a class="dropdown-item" id="check_status" href="' . route('sliders.remove_outlet', $row->id) . '">Remove Outlets</a>';
                }
                if ($row->status == StatusEnum::Bannar->value) {
                    $html .= '<a class="dropdown-item" id="check_status" href="' . route('sliders.remove_bannar', $row->id) . '">Remove Bannar</a>';
                }
