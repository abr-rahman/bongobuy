    ...
    <div>
        <div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col-10">
                        {{ __('Offline Sales Report') }}
                    </div>
                    <div class="col-2">
                        <select id="filterSelectOffline" class="form-control">
                            <option value="">Select by Filter</option><hr><hr> <br>
...
@push('js')
    <script>
        ...
        var orders_table = $('.table-data').DataTable({
            ...
            "ajax": {
                "url": "{{ route('offline.report.sales') }}"
            },
        ...
        $(document).ready(function () {
            $('body').on('change', '#filterSelectOffline', function(e) {
                var selectedOption = $(this).val();
                $.ajax({
                    type: 'GET',
                    url: "{{ route('filter.route.offline') }}",
                    ...
                    success: function (data) {
                        ...
                        if (data.length > 0) {
                            $.each(data, function (index, item) {
                                ...
                                row.append('<td>' + item.payable_amount + '</td>');
                                row.append('<td>' + item.due_amount + '</td>');
