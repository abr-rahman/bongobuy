        ...
        <div class="mx-auto w-full xl:max-w-screen-xl px-6 py-10">
            <div>
                <div class="grid md:grid-cols-2 gap-10">
                    <div class="w-full">
                        <div id="productMainThumbnail">
                            <img src="{{ asset('uploads/product/' . $product->product_image) ?? asset($product->product_image) }}"
                                alt="img" class="parentImg w-full max-h-[300px] object-contain border">
                        </div>

                        <div class="grid grid-cols-4 w-full" id="colorImagesContainer">
                            @if (isset($product->thumbnail_image) == 'default-product-image.png')
                                <img onclick="productDetailsImg(this)"
                                src="{{ asset('uploads/product/' . $product->product_image) ?? asset($product->product_image) }}"
                    ...
                    <div class="flex flex-col gap-5">
                        ...
                        <div class="grid gap-5">
                                ...
                                <div class="flex items-center gap-2">
                                    <div class="flex">
                                        @for ($i = 1; $i <= 5; $i++)
                                            ...
                                            @else
                                                <svg xmlns="http://www.w3.org/2000/svg" class="text-slate-300"
                                                    width="16" height="16" viewBox="0 0 24 24" stroke-width="2"
                                                    stroke="currentColor" fill="none" stroke-linecap="round"
                                                    stroke-linejoin="round">
                                                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                    ...
                                    <span style="color: #0025b8">({{ $reviews->count() }} customer review)</span>
                                </div>
                                <div class="flex items-center gap-5">
                                    {{-- <img src="{{ asset('frontend/images/share.png') }}" alt="img" style="width: 22px; "> --}}
                                    <button class="">
                                        {{-- <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30"
                                            viewBox="0 0 24 24" fill="none" stroke="#969696" stroke-width="1"
                            ...
                            <div class="flex items-center">
                                <div style="padding: 0 10px 0 0; border-right: 1px solid #969696; white-space: nowrap; ">
                                    Brand: <a style="color: #0025b8" href="">{{ $product->brand->name ?? '' }}</a></div>
                                <div style="padding: 0 0 0 10px; color: #0025b8">
                                    <a href="{{ route('categories.wise_page', ['slug' => $product?->brand?->slug, 'type' => 'brand']) }}">More Products From <strong>{{ $product?->brand?->name ?? '' }}</strong> Brand</a>
                                </div>
                            </div>
                            <div class="px-5">
                                <p class="">{!! $product->short_description !!}</p>
                            </div>
                            @if ($inventory && isset($totalQuantity))
                                @if ($totalQuantity > 0)
                                    <p style="color: #08c551; font-weight: 600">Stock In
                                        <span id="colorQuantity">({{ $totalQuantity > 30 ? 30 : $totalQuantity }})</span>
                                    </p>
                                @else
                            ...
                            <div class="flex items-center flex-wrap  gap-5 sm:gap-2">
                                <div class="w-full sm:w-auto flex items-center justify-between gap-5 border p-2">
                                    <span>Quantity</span>
                                    <div class="flex items-center">
                                        <div id="decBtn">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="cursor-pointer" width="24"
                                        ...
                                        <input type="number" class="w-10 p-1 outline-none text-center"
                                            @if (isset($allCart[$product->id]['quantity'])) value="{{ $allCart[$product->id]['quantity'] }}"
                                        @else
                                            value="1" @endif
                                ...
                                <div class="w-full sm:w-auto border p-2 flex items-center gap-2"
                                    style="justify-content: space-evenly">
                                        @foreach ($productColors as $productColor)
                                        <div style="padding: 8px; cursor: pointer; box-shadow: rgba(0, 0, 0, 0.15) 0px 2px 8px;">
                                            @if ($productColor->color)
                                                <span class="color-item" title="{{ $productColor->color->color_name ?? 'n/a'}}" data-color-id="{{ $productColor->color->id }}"
                                                    onclick="colorImages($(this))" style="background: {{ $productColor->color->color_code ?? '#dddd' }}; padding: 10px; display: block;"></span>
                                            @else
                                                <span>No color associated</span>
                                            @endif
                                        </div>
                                    @endforeach
                                    </div>
                            ...
                            <div class="grid sm:flex items-center gap-5 sm:gap-0">
                                @if($productColors->count() != 0)
                                    <a href="{{ route('addproduct.buy_now', $product->id) }}" id=""
                                        class="border border-bg-fave-500 font-semibold text-white py-3 px-8 bg-green-500 text-center">Buy
                                        Now</a>
                                    <a href="{{ route('addproduct.to.cart', $product->id) }}" id="addCartUpdate"
                                        class="border border-bg-fave-500 font-semibold text-white py-3 px-8 bg-fave-500 text-center">Add
                                        To Cart</a>
                                @else
                                    <a href="{{ route('without.variant.buy_now', $product->id) }}" id=""
                                        class="border border-bg-fave-500 font-semibold text-white py-3 px-8 bg-green-500 text-center">Buy
                                        Now</a>
                                    <a href="{{ route('without.variant.cart', $product->id) }}" id="addCartUpdate"
                                        class="border border-bg-fave-500 font-semibold text-white py-3 px-8 bg-fave-500 text-center">Add
                                        To Cart</a>
                                @endif
                        ...
                        @foreach ($relatedProducts as $relatedProduct)
                            <div class="bg-white pb-5 sm:p-5 grid gap-2 border-b sm:border-b-0 sm:border-r">
                                <div class="flex justify-between">
                                    <a href="{{ route('categories.wise_page', ['slug' => $relatedProduct->category->slug, 'type' => 'category']) }}"
                                ...
                                <a href="{{ route('frontend.product.details', ['slug' => $relatedProduct->slug, 'id' => $relatedProduct->id]) }}"
                                    class="line-clamp-2 text-sm hover:text-fave-500 cursor-pointer">
                                    {{ $relatedProduct->product_name }}</a>
                                <div class="flex justify-between">
                                    @if ($relatedProduct->status == App\Enums\ProductStatus::Comming->value)
                                        <button
                                            class="slider-button bg-fave-500 px-2 hover:shadow-lg hover:shadow-fave-100 rounded border-b-2 border-fave-700 text-white font-semibold">Coming
                                            Soon </button>
                                    @else
                                        <div class="flex items-center gap-3">
                                            <p class="font-semibold text-lg">&#2547;.
                                                {{ $relatedProduct->userPrice->discount_fixed }}</p>
                                            @if ($relatedProduct->userPrice->discount_fixed != $relatedProduct->userPrice->regular_price)
                                                <del class="text-slate-400 text-sm">&#2547;.
                                                    {{ $relatedProduct->userPrice->regular_price }}</del>
...
@push('js')
    <script>
        ...
        function colorImages(elem){
            color_id = elem.data('color-id');
            var product_id = '{{ $product->id }}';
            $.ajax({
                type: 'POST',
                url: "{{ route('select.color.images') }}",
                data: {
                    color_id: color_id,
                    product_id: product_id
                },
                success: function(data) {
                    var colorImagesContainer = $('#colorImagesContainer');
                    var productMainThumbnail = $('#productMainThumbnail');
                    var colorQuantity = $('#colorQuantity').html('('+data.showColorQty+')');
                    colorImagesContainer.empty(); // Clear existing images
                    productMainThumbnail.empty(); // Clear existing images

                    var images = data.images;
                    if (images && images.length > 0) {
                        var imgElementThumbnail = $('<img>')
                            .attr('src', "{{ asset('uploads/product/color/') }}/" + images[0])
                            .attr('alt', 'Image')
                            .addClass('parentImg w-full max-h-[300px] object-contain border')
                            .on('click', function() {
                                productDetailsImg(this);
                            });
                        productMainThumbnail.append(imgElementThumbnail);
                        $.each(images, function(index, image) {
                            var imgElement = $('<img>')
                                .attr('src', "{{ asset('uploads/product/color/') }}/" + image)
                                .attr('alt', 'Image')
                                .addClass('w-full max-h-[120px] object-contain border p-1')
                                .on('click', function() {
                                    productDetailsImg(this);
                                });
                            colorImagesContainer.append(imgElement);
                        });
                    } else {
    ...
    <script>
        document.querySelectorAll('.color-item').forEach(item => {
            item.addEventListener('click', function() {
                document.querySelectorAll('.color-item').forEach(i => i.style.border = '1px solid #fff');
                this.style.border = '1px solid #fcac19';
            });
        });
    </script>
@endpush
