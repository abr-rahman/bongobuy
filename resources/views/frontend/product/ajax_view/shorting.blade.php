...
{{-- <div class="bg-white pb-5 sm:p-5 grid gap-2 border-b sm:border-b-0 sm:border-r">
    <div class="flex justify-between">
        <a href="{{ route('categories.wise_page', ['slug' => $product->category->slug, 'type' => 'category']) }}" class="hover:text-fave-500 py-1 my-1 ">{{ $product->category->name }}</a>
    ...
    <a href="{{ route('frontend.product.details', ['slug' => $product->slug, 'id' => $product->id]) }}" class="line-clamp-2 text-sm hover:text-fave-500 cursor-pointer">{{ $product->product_name }}</a>
    @if ($product->status == App\Enums\ProductStatus::Comming->value)
    <button class="slider-button bg-fave-500 px-5 py-2 hover:shadow-lg hover:shadow-fave-100 rounded border-b-2 border-fave-700 text-black font-semibold">Coming
        Soon </button>
    @else
    <div class="flex justify-between">
